...
`{APP_BASE_URL}/auth/callback/naver`를 등록하고 Client ID, Client Secret, Callback URL을 설정합니다.
AWS 운영은 Git에서 제외된 `src/main/resources/application-naver-private.yml`을 이미지에 포함합니다. 기능이 비활성화된 환경에서는
...
- SMART SHOP 제품 선택·수량·사양 문의와 관리자 등록 가격 기반 토스페이먼츠 테스트 구매
- 별도 SHOP 관리자 권한과 화면 진입 시 필수 비밀번호 재확인, 제품 25개의 가격·대표 이미지·공개 상태 관리
- 토스페이먼츠 테스트·라이브 모드, 주문 생성, 서버 금액 검증, 멱등 승인, 실패·사용자취소 기록, 영수증·운영 알림 및 관리자 전액 취소
...
- SHOP 관리자: `http://localhost:8080/shop-admin-entry.html` (아이디 없이 별도 2차 비밀번호 입력)

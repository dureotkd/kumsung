...
public class NaverOAuth2Configuration {
    ...
    ClientRegistrationRepository naverClientRegistrationRepository(
        ...
        @Value("${app.naver.callback-url:}") String callbackUrl,
        @Value("${app.base-url}") String baseUrl){
        if(!StringUtils.hasText(clientId)||!StringUtils.hasText(clientSecret))
            throw new IllegalStateException("네이버 로그인 사용 시 Client ID와 Client Secret이 필요합니다.");
        String normalizedBaseUrl=baseUrl.replaceAll("/+$","");
        String redirectUri=StringUtils.hasText(callbackUrl)
            ? callbackUrl.trim()
            : normalizedBaseUrl+"/auth/callback/naver";

...
async function loadSupportContent(){
  const cases=await fetchContent("CONSTRUCTION_CASE");
  supportContentRows=new Map(cases.map(row=>[Number(row.id),row]));
...
function openContentDetail(id){
  ...
  document.querySelector("#contentDetailType").textContent="시공사례";document.querySelector("#contentDetailTitle").textContent=row.title;
...
loadSupportContent().catch(error=>{constructionCaseGrid.innerHTML=`<p class="sync-empty">${contentEsc(error.message)}</p>`});
